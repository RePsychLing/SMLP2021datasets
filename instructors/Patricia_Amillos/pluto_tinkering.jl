...
My main concern is that I am working with categorical independent and dependent variables, specifically those from a binomial distribution. Most courses I have attended only presented experiments with continuous variables as an example. Although they mention that it is only a matter of using different function in R (glmer instead of lmer), it would still be nice to see how one would set a model for data like this and interpret the outcomes of the model. 
"""
...
I would also like to know how to interpret the variance and standard deviations in the random effects when the dependent variable is binomial. It is very abstract to me how to translate this variation in relation to a variable that can only take values from 0 to 1. What would be a reasonable value for variance in this case?
"""
...
For the model that returned no warnings, I plotted its residuals and found that it does not follow a normal curve/qqplot. The histogram appears to be bimodal and the qpplot shows a gap between the curve. Does it have something to do with my independent variables being categorical as well? Or does the data require transformation?

"""

# ╔═╡ 74ee43b5-3cf8-4876-bb21-3c06f017d231
md"""
### Intepreting Fixed Effects

When interpreting the fixed effects of the model, I followed Bodo Winter’s advice in his book to convert the log odds into probabilities. I understand that it tells you the probability of the variable taking a specific value or not (in my model, it is the probability that a person will get a correct response after they underwent the training). However, I’m not sure how to interpret these along with the p-values that the fixed effects also return. So for example I wrote this finding: “In the post-test, the predicted probability of a correct response for trained items is 0.50482 (p >0.05)”

- Is it possible to report this as: Participants on average were likely to discriminate the sounds correctly 50% of the time after training. The p-value is larger than 0.05 and thus shows that there is no difference to their performance before training. Am I correct in assuming: 
- that the converted log odds function similarly as the result of doing a one-sample t-test and comparing the scores to chance performance?
- that the p-value reported tells me the difference between the conditions I specified in the model?
- This question ties in to my next one. Which outputs of the fixed effects do I interpret to answer the questions I’m interested in:
- Are these log odds above chance?
- Is there a difference between the pre and post test accuracy?
- Is there a difference between the accuracy of trained and untrained items?

"""

# ╔═╡ 5fafbc4f-01c4-4fad-b2ec-2f3abe939891
md"""
#### Contrast Coding
I would like feedback on my choice of contrast coding to reflect the hypotheses I set. I would also like feedback on my rationale for setting and simplifying the random effects structure when a model returns a singular fit

I also have a second data set from participants where we obtain the accuracy of participants on selecting the item that matches the words they learned. Some trials are harder than the others (i.e. when they have a choice of two objects that form a minimal pair distinguishable only if you’ve learned the non-native Hindi contrast). In contrast, the easier trials have an object that doesn’t form a minimal pair with the target as a foil and thus requires no discrimination abilities. I would like to compare if their accuracy for the easier and harder trials are each above chance and if one condition is better than the other.
"""

# ╔═╡ 8c50b57c-97e8-4fda-b126-1224b4bfdb3a
md"""

#### Comparing accuracy to chance

I’ve opted to just do a t-test because this is the only way I know how to compare accuracy to chance performance, but the flaw is that I have to aggregate the data by participant. I found this analysis in MacDonald et al. (2017) wherein they compare chance using a linear model, but I’m not sure how to apply that to my data. I’ve appended the first paragraph on p.70 under section “2.2.2 Exposure Trials” that describes this analysis, and the paper is also in the repository.

Considering that I am also interested in comparing accuracy to chance level performance in my AXB data set, should I also follow a model similar to this one rather than the one I’ve made? 
"""
...
git-tree-sha1 = "ab3f52bd391aebdbe85c0d8b521d1aecdf138c9f"
